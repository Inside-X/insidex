...
describe('leads.routes',()=>{test('handlers', async()=>{
 const authorizeRole=jest.fn(()=>pass);
 const routes=await loadRoute('../../src/routes/leads.routes.js',{
 ...
 let res={status:jest.fn(()=>res),json:jest.fn()}; await runHandlers(routes.find(r=>r.method==='post').handlers,{},res); expect(res.status).toHaveBeenCalledWith(201);
 res={status:jest.fn(()=>res),json:jest.fn()}; await runHandlers(routes.find(r=>r.method==='get').handlers,{},res); expect(res.status).toHaveBeenCalledWith(200); expect(authorizeRole).toHaveBeenCalledWith('admin');
});});