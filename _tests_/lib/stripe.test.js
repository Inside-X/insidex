...
describe('lib/stripe', () => {
  ...
  test('constructEvent success', () => {
    const raw=Buffer.from(JSON.stringify({id:'evt'})); const secret='sec'; const sig=sigFor(raw,secret);
  ...
  test('input validation and malformed payload/signature branches', () => {
    ...
    const oldTs=1; const raw=Buffer.from('{}'); const sig=sigFor(raw,'s',oldTs);
    expect(()=>stripe.webhooks.constructEvent(raw,sig,'s',{toleranceSeconds:1})).toThrow('outside tolerance');
    const sig2=`t=${Math.floor(Date.now()/1000)},v1=abc`;  expect(()=>stripe.webhooks.constructEvent(raw,sig2,'s')).toThrow('No signatures found matching');
    const rawBad=Buffer.from('{'); const sigBad=sigFor(rawBad,'s'); expect(()=>stripe.webhooks.constructEvent(rawBad,sigBad,'s')).toThrow('Malformed webhook payload');
  });
});